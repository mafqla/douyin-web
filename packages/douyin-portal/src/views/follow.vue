...
const checkAndLoadMore = async () => {
  // 过滤掉直播用户后的数量
  const liveUserIds = new Set(liveList.value.map(item => item.room?.owner?.id_str))
  const filteredCount = followingList.value.filter(user => !liveUserIds.has(user.uid)).length
  
  // 如果过滤后不足30个且还有更多数据，继续加载
  if (filteredCount < 30 && followingHasMore.value && !followingLoadingMore.value) {
...
const getData = async (count: number) => {
  try {
    ...
    const awemeList = res.data?.map(item => item.aweme as unknown as IAwemeInfo) || []
...
const selectUser = async (userItem: IFollowingUser) => {
  ...
  try {
    ...
    if (notSeenIds && notSeenIds.length > 0) {
      // 找到第一个未看的视频
      const notSeenVideo = videoList.find(v => notSeenIds.includes(v.aweme_id))
      if (notSeenVideo) {
...
const getAvatarUrl = (userItem: IFollowingUser) => {
  return userItem.avatar_thumb?.url_list?.[0] || 
...
const filteredFollowingList = computed(() => {
  const liveUserIds = new Set(liveList.value.map(item => item.room?.owner?.id_str))
  const filtered = followingList.value.filter(user => !liveUserIds.has(user.uid))
  // 保持30个
  ...
  <div class="follow-page">
    ...
    <div class="following-sidebar" :class="{ collapsed: sidebarCollapsed }">
      ...
      <div class="sidebar-header" :class="{ collapsed: sidebarCollapsed }">
        <div v-if="!sidebarCollapsed">{{ hasLive ? `正在直播(${liveCount})` : `精选关注人(${followingCount})` }}</div>
        <div class="collapse-btn" @click="toggleSidebar">
          <!-- 展开图标 -->
          <svg v-if="sidebarCollapsed" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M14 5.25H3V6.75H14V5.25ZM3 11.25H14V12.75H3V11.25ZM3 17.25H14V18.75H3V17.25Z" fill="currentColor"></path>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M20.2616 11.9999L17.0471 16.0423L18.1876 17.0094L21.5302 12.8059C21.9027 12.3374 21.9027 11.6624 21.5302 11.194L18.1876 6.99042L17.0471 7.95755L20.2616 11.9999Z" fill="currentColor"></path>
          </svg>
          <!-- 收起图标 -->
          <svg v-else viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M10 5.25H21V6.75H10V5.25ZM21 11.25H10V12.75H21V11.25ZM21 17.25H10V18.75H21V17.25Z" fill="currentColor"></path>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M3.73837 11.9999L6.95287 16.0423L5.81244 17.0094L2.46976 12.8059C2.09726 12.3374 2.09726 11.6624 2.46976 11.194L5.81244 6.99042L6.95287 7.95755L3.73837 11.9999Z" fill="currentColor"></path>
          </svg>
        </div>
      </div>
      ...
      <div class="sidebar-content" @scroll="onFollowingScroll">
        ...
        <template v-if="hasLive">
          <ul class="live-list" :class="{ 'collapsed-list': sidebarCollapsed }">
            <li 
              ...
              class="live-item"
              :class="{ 'collapsed-item': sidebarCollapsed, active: selectedLiveItem?.room.id_str === item.room.id_str }"
              @mouseenter="onLiveItemHover(item, index)"
              @mouseleave="onLiveItemLeave"
            >
              <div class="avatar-wrapper">
                <div class="avatar live-avatar" :class="{ 'collapsed-avatar': sidebarCollapsed }">
                  <img :src="getLiveAvatarUrl(item)" :alt="item.room.owner.nickname" />
        ...
        <Loading :show="followingLoading" :isShowText="false">
          <ul class="following-list" :class="{ 'collapsed-list': sidebarCollapsed }">
            <li 
              v-for="item in filteredFollowingList" 
              :key="item.uid"
              class="following-item"
              :class="{ active: selectedUserId === item.uid, 'collapsed-item': sidebarCollapsed }"
              @click="selectUser(item)"
            >
              <div class="avatar-wrapper">
                <div class="avatar" :class="{ 'collapsed-avatar': sidebarCollapsed }">
              ...
              <div class="user-info" v-if="!sidebarCollapsed">
                ...
                <div class="unseen-count" v-if="item.not_seen_item_id_list_v2 && item.not_seen_item_id_list_v2.length > 0">
                  <span>{{ item.not_seen_item_id_list_v2.length > 99 ? '99+' : item.not_seen_item_id_list_v2.length }}个作品未看</span>
                </div>
              </div>
            </li>
            <!-- 加载更多提示 -->
